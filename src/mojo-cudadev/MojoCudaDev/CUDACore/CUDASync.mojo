...
@always_inline
fn syncthreads_and(ok: Bool) -> Bool:
    """Block-wide barrier + AND-reduce: true only if every thread in the block
    passed true. Mirrors CUDA `__syncthreads_and(predicate)`.
    """
    from std.gpu import thread_idx
