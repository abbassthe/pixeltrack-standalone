...
alias karimaki_circle_fit = riemannFit.CircleFit
...
fn multScatt(
    ...
    # limit R to 20GeV...
    var _ = layer
    var pt2 = min(20.0, bField * radius)
    pt2 *= pt2
    ...
    var fact = geometry_factor * riemannFit.sqr(13.6 / 1000.0)
    return fact / (pt2 * (1.0 + riemannFit.sqr(slope))) * (abs(length) * XXI_0) * riemannFit.sqr(1.0 + 0.038 * math.log(abs(length) * XXI_0))
...
fn rotationMatrix(slope: Float64) -> riemannFit.Matrix2d:
    var Rot = riemannFit.Matrix2d()
    Rot[0, 0] = 1.0 / math.sqrt(1.0 + riemannFit.sqr(slope))
...
fn translateKarimaki(mut circle: karimaki_circle_fit, x0: Float64, y0: Float64, mut jacobian: riemannFit.Matrix3d):
    var A: Float64 = 0.0
    var U: Float64 = 0.0
    var BB: Float64 = 0.0
    var C: Float64 = 0.0
    ...
    A = 2.0 * DO + circle.par[2] * (riemannFit.sqr(DO) + riemannFit.sqr(DP))
    U = math.sqrt(1.0 + circle.par[2] * A)
    xi = 1.0 / (riemannFit.sqr(BB) + riemannFit.sqr(C))
    v = 1.0 + circle.par[2] * DO
    lambda_ = (0.5 * A) / (U * riemannFit.sqr(1.0 + U))
    mu = 1.0 / (U * (1.0 + U)) + circle.par[2] * lambda_
    zeta = riemannFit.sqr(DO) + riemannFit.sqr(DP)

    jacobian[0, 0] = xi * uu * v
    jacobian[0, 1] = -xi * riemannFit.sqr(circle.par[2]) * DP
    jacobian[0, 2] = xi * DP
...
fn prepareBrokenLineData[M3xN: MatrixLike, V4: MatrixLike, N: Int](
    ...
    if riemannFit.cross2D(d, e) > 0:
        results.qCharge = -1
    else:
        results.qCharge = 1

    var slope: Float64 = -Float64(results.qCharge) / fast_fit[3, 0].cast[DType.float64]()
    ...
    while i < n:
        ...
        results.sTransverse[i] = Float64(results.qCharge) * fast_fit[2, 0].cast[DType.float64]() * math.atan2(riemannFit.cross2D(d, e), d.dot(e))
        i += 1
    var z : riemannFit.VectorNd[N] = hits.block[1, N](2, 0).transpose().cast[DType.float64]()

    var pointsSZ: riemannFit.Matrix2xNd[N] = riemannFit.Matrix2xNd[N].Zero()
    i = 0
    while i < n:
        ...
        i += 1
    results.sTotal = pointsSZ.block[1, N](0, 0).transpose()
    results.zInSZplane = pointsSZ.block[1, N](1, 0).transpose()

    results.varBeta[0] = 0
    results.varBeta[n - 1] = 0
    i = 1
    while i < n - 1:
        results.varBeta[i] = multScatt(results.sTotal[i + 1] - results.sTotal[i], B, fast_fit[2, 0].cast[DType.float64](), i + 2, slope) + multScatt(results.sTotal[i] - results.sTotal[i - 1], B, fast_fit[2, 0].cast[DType.float64](), i + 1, slope)
        i += 1
...
fn matrixC_u[
    ...
    weights: riemannFit.VectorNd[N],
    sTotal: riemannFit.VectorNd[N],
    varBeta: riemannFit.VectorNd[N],
) -> riemannFit.MatrixNd[N]:
    var n: Int = N
    var i: Int = 0

    var C_U = riemannFit.MatrixNd[N].Zero()
    i = 0
    while i < n:
        C_U[i, i] = weights[i]
        if i > 1:
            C_U[i, i] += 1.0 / (varBeta[i - 1] * riemannFit.sqr(sTotal[i] - sTotal[i - 1]))
        if i > 0 and i < n - 1:
            C_U[i, i] += (1.0 / varBeta[i]) * riemannFit.sqr((sTotal[i + 1] - sTotal[i - 1]) / ((sTotal[i + 1] - sTotal[i]) * (sTotal[i] - sTotal[i - 1])))
        if i < n - 2:
            C_U[i, i] += 1.0 / (varBeta[i + 1] * riemannFit.sqr(sTotal[i + 1] - sTotal[i]))

        if i > 0 and i < n - 1:
            C_U[i, i + 1] = 1.0 / (varBeta[i] * (sTotal[i + 1] - sTotal[i])) * (-(sTotal[i + 1] - sTotal[i - 1]) / ((sTotal[i + 1] - sTotal[i]) * (sTotal[i] - sTotal[i - 1])))
        if i < n - 2:
            C_U[i, i + 1] += 1.0 / (varBeta[i + 1] * (sTotal[i + 1] - sTotal[i])) * (-(sTotal[i + 2] - sTotal[i]) / ((sTotal[i + 2] - sTotal[i + 1]) * (sTotal[i + 1] - sTotal[i])))

        if i < n - 2:
            C_U[i, i + 2] = 1.0 / (varBeta[i + 1] * (sTotal[i + 2] - sTotal[i + 1]) * (sTotal[i + 1] - sTotal[i]))
...
fn fastFit[M3xN: MatrixLike, V4: MatrixLike](hits: M3xN, mut result: V4):
    var N = M3xN.ColsAtCompileTime()
    var n = N

    var a: riemannFit.Vector2d = (hits.block[2, 1](0, n // 2) - hits.block[2, 1](0, 0)).cast[DType.float64]()
    var b: riemannFit.Vector2d = (hits.block[2, 1](0, n - 1) - hits.block[2, 1](0, n // 2)).cast[DType.float64]()
    var c: riemannFit.Vector2d = (hits.block[2, 1](0, 0) - hits.block[2, 1](0, n - 1)).cast[DType.float64]()

    var tmp = 0.5 / riemannFit.cross2D(c, a)
    ...
    result[2, 0] = (math.sqrt(a.squaredNorm() * b.squaredNorm() * c.squaredNorm()) / (2.0 * abs(riemannFit.cross2D(b, a)))).cast[V4.ElemType]()

    var d : riemannFit.Vector2d= hits.block[2, 1](0, 0).cast[DType.float64]() - result.head[2]().cast[DType.float64]()
    var e : riemannFit.Vector2d= hits.block[2, 1](0, n - 1).cast[DType.float64]() - result.head[2]().cast[DType.float64]()

    result[3, 0] = (result[2, 0].cast[DType.float64]() * math.atan2(riemannFit.cross2D(d, e), d.dot(e)) / (hits[2, n - 1].cast[DType.float64]() - hits[2, 0].cast[DType.float64]())).cast[V4.ElemType]()
...
fn circleFit[M3xN: MatrixLike, M6xN: MatrixLike, V4: MatrixLike, N: Int](
    ...
    var s = data.sTransverse
    var S = data.sTotal
    var Z = data.zInSZplane
    var VarBeta = data.varBeta
    var slope: Float64 = -Float64(circle_results.qCharge) / fast_fit[3, 0].cast[DType.float64]()
    VarBeta *= 1.0 + riemannFit.sqr(slope)
    ...
    var V = riemannFit.Matrix2d()
    var w = riemannFit.VectorNd[N]()
    var RR = riemannFit.Matrix2d()
    i = 0
    while i < n:
        ...
        i += 1
    var r_u : riemannFit.VectorNplusONEd[N] = riemannFit.VectorNplusONEd[N]()
    r_u[n] = 0
    i = 0
    while i < n:
        r_u[i] = w[i] * Z[i]
    ...
    var C_U : riemannFit.MatrixNplusONEd[N] = riemannFit.MatrixNplusONEd[N]()
    ...
    while i < n:
        ...
        C_U[n, i] = C_U[i, n]
        if i > 0 and i < n - 1:
            C_U[n, n] += riemannFit.sqr(s[i + 1] - s[i - 1]) / (4.0 * VarBeta[i])
        i += 1
    ...
    var I := riemannFit.MatrixNplusONEd[N]()
    ...
    var d : riemannFit.Vector2d = hits.block[2, 1](0, 0).cast[DType.float64]() + (-Z[0] + u[0]) * radii.block[2, 1](0, 0)
    var e : riemannFit.Vector2d = hits.block[2, 1](0, 1).cast[DType.float64]() + (-Z[1] + u[1]) * radii.block[2, 1](0, 1)

    circle_results.par[0] = math.atan2((e - d)[1], (e - d)[0])
    circle_results.par[1] = -Float64(circle_results.qCharge) * (fast_fit[2, 0].cast[DType.float64]() - math.sqrt(riemannFit.sqr(fast_fit[2, 0].cast[DType.float64]()) - 0.25 * (e - d).squaredNorm()))
    circle_results.par[2] = Float64(circle_results.qCharge) * (1.0 / fast_fit[2, 0].cast[DType.float64]() + u[n])

    debug_assert(Float64(circle_results.qCharge) * circle_results.par[1] <= 0)

    var eMinusd : riemannFit.Vector2d = e - d
    var tmp1 = eMinusd.squaredNorm()

    var jacobian: riemannFit.Matrix3d = riemannFit.Matrix3d()
    jacobian[0, 0] = (radii[1, 0] * eMinusd[0] - eMinusd[1] * radii[0, 0]) / tmp1
    jacobian[0, 1] = (radii[1, 1] * eMinusd[0] - eMinusd[1] * radii[0, 1]) / tmp1
    jacobian[0, 2] = 0.0
    jacobian[1, 0] = (Float64(circle_results.qCharge) / 2.0) * (eMinusd[0] * radii[0, 0] + eMinusd[1] * radii[1, 0]) / math.sqrt(riemannFit.sqr(2.0 * fast_fit[2, 0].cast[DType.float64]()) - tmp1)
    jacobian[1, 1] = (Float64(circle_results.qCharge) / 2.0) * (eMinusd[0] * radii[0, 1] + eMinusd[1] * radii[1, 1]) / math.sqrt(riemannFit.sqr(2.0 * fast_fit[2, 0].cast[DType.float64]()) - tmp1)
    jacobian[1, 2] = 0.0
    jacobian[2, 0] = 0.0
    jacobian[2, 1] = 0.0
    jacobian[2, 2] = Float64(circle_results.qCharge)
    ...
    circle_results.cov[0, 0] += (1.0 + riemannFit.sqr(slope)) * multScatt(S[1] - S[0], B, fast_fit[2, 0].cast[DType.float64](), 2, slope)
    ...
    i = 0
    while i < n:
        circle_results.chi2 += Float32(w[i] * riemannFit.sqr(Z[i] - u[i]))
        if i > 0 and i < n - 1:
            circle_results.chi2 += Float32(riemannFit.sqr(u[i - 1] / (s[i] - s[i - 1]) - u[i] * (s[i + 1] - s[i - 1]) / ((s[i + 1] - s[i]) * (s[i] - s[i - 1])) + u[i + 1] / (s[i + 1] - s[i]) + (s[i + 1] - s[i - 1]) * u[n] / 2.0) / VarBeta[i])
        i += 1
...
#The step 3 is the correction of the fast pre-fitted parameters for the innermost part of the track. It is first done in a comfortable coordinate system (the one in which the first hit is the origin) and then the parameters and their covariance matrix are transformed to the original coordinate system.
fn lineFit[V4: MatrixLike, M6xN: MatrixLike, N: Int](
    hits_ge: M6xN, fast_fit: V4, B: Float64, data: PreparedBrokenLineData[N], mut line_results: riemannFit.LineFit
):
    var n: Int = N
    var i: Int = 0
    ...
    var slope: Float64 = -Float64(data.qCharge) / fast_fit[3, 0].cast[DType.float64]()
    var R = rotationMatrix(slope)

    var V : riemannFit.Matrix3d = riemannFit.Matrix3d.Zero()
    var JacobXYZtosZ : riemannFit.Matrix2x3d = riemannFit.Matrix2x3d.Zero()
    var w :riemannFit.VectorNd[N]= riemannFit.VectorNd[N].Zero()
    i = 0
    while i < n:
        V[0, 0] = hits_ge.col(i)[0].cast[DType.float64]()
        V[0, 1] = hits_ge.col(i)[1].cast[DType.float64]()
        ...
        i += 1
    var r_u : riemannFit.VectorNd[N] = riemannFit.VectorNd[N]()
    i = 0
    while i < n:
        r_u[i] = w[i] * Z[i]
        i += 1
    ...
    var I : riemannFit.MatrixNd[N] = riemannFit.MatrixNd[N]()
    ...
    var u : riemannFit.VectorNd[N] = I * r_u

    line_results.par[0] = (u[1] - u[0]) / (S[1] - S[0])
    line_results.par[1] = u[0]
    var idiff = 1.0 / (S[1] - S[0])
    line_results.cov[0, 0] = (I[0, 0] - 2 * I[0, 1] + I[1, 1]) * riemannFit.sqr(idiff) + multScatt(S[1] - S[0], B, fast_fit[2, 0].cast[DType.float64](), 2, slope)
    line_results.cov[0, 1] = (I[0, 1] - I[0, 0]) * idiff
    line_results.cov[1, 0] = (I[0, 1] - I[0, 0]) * idiff
    line_results.cov[1, 1] = I[0, 0]

    var jacobian = riemannFit.Matrix2d()
    jacobian[0, 0] = 1.0
    jacobian[0, 1] = 0.0
    jacobian[1, 0] = -S[0]
    jacobian[1, 1] = 1.0
    ...
    i = 0
    while i < n:
        line_results.chi2 += w[i] * riemannFit.sqr(Z[i] - u[i])
        if i > 0 and i < n - 1:
            line_results.chi2 += riemannFit.sqr(u[i - 1] / (S[i] - S[i - 1]) - u[i] * (S[i + 1] - S[i - 1]) / ((S[i + 1] - S[i]) * (S[i] - S[i - 1])) + u[i + 1] / (S[i + 1] - S[i])) / VarBeta[i]
        i += 1
...
#
#\warning see circleFit(), lineFit() and Fast_fit() warnings.
#
#\bug see circleFit(), lineFit() and Fast_fit() bugs.
#
...
fn helixFit[N: Int](
    ...
    var jacobian = riemannFit.Matrix3d()

    prepareBrokenLineData(hits, fast_fit, B, data)
    lineFit(hits_ge, fast_fit, B, data, line)
    circleFit(hits, hits_ge, fast_fit, B, data, circle)
    ...
    jacobian[2, 0] = 0.0
    jacobian[2, 1] = 0.0
    jacobian[2, 2] = -abs(circle.par[2]) * B / (riemannFit.sqr(circle.par[2]) * circle.par[2])
