# Mojo port of plugin-PixelTriplets/CircleEq.h.
#
...
struct CircleEq[T: DType](Copyable, Movable):
    ...
    fn dca(self, x: Scalar[Self.T], y: Scalar[Self.T]) -> Scalar[Self.T]:
        var xx = self.m_c * (self.m_xp - x) + self.m_alpha
        var yy = self.m_c * (self.m_yp - y) + self.m_beta
        return sqrt(xx * xx + yy * yy) - Scalar[Self.T](1)

    # curvature
    fn curvature(self) -> Scalar[Self.T]:
        return self.m_c

    # alpha and beta
    fn cosdir(self) -> Tuple[Scalar[Self.T], Scalar[Self.T]]:
        return (self.m_alpha, self.m_beta)

    # alpha and beta at given point
    fn cosdir(
        self, x: Scalar[Self.T], y: Scalar[Self.T]
    ) -> Tuple[Scalar[Self.T], Scalar[Self.T]]:
        return (
    ...
    # center
    fn center(self) -> Tuple[Scalar[Self.T], Scalar[Self.T]]:
        return (
            self.m_xp + self.m_alpha / self.m_c,
            self.m_yp + self.m_beta / self.m_c,
        )

    fn radius(self) -> Scalar[Self.T]:
